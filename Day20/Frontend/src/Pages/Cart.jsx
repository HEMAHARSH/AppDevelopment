...
const Cart = () => {
  ...
  const billingamount = async() => {
    ...
    await axios.post("http://localhost:8080/api/v1/auth/post",{amount:totalPrice,billingstatus:"paid",name:username,quantity:totalQuantity});
    nav(`/bill`);
  };
  const billingcancel = async() => {
    await axios.post("http://localhost:8080/api/v1/auth/post",{amount:totalPrice,billingstatus:"cancelled",name:username,quantity:totalQuantity})
    nav(`/home`);
    dispatch(clearCart());
  };
