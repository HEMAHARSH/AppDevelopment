...
function Signup() {
  ...
  const [error, setError] = useState(false); // Changed 'seterror' to 'setError' for consistency
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (username === "admin" && password === "admin@123") {
        ...
        dispatch(login(username));
        const response = await axios.post("http://localhost:8999/api/v1/auth/authenticate", { username, password });
    ...
    <div className="body">
      ...
      <form onSubmit={handleSubmit} className="cover1">
        ...
        <input
          ...
          value={username}
          onChange={(e) => setUsername(e.target.value)} // Removed 'on'
        />
        <span>
        ...
        <input
          ...
          value={password}
          onChange={(e) => setPassword(e.target.value)} // Removed 'on'
        />
        <span>
        ...
        <h2 className="account">Don't have an account?</h2>
