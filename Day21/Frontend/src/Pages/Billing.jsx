...
const CartPage = () => {
  ...
  const isAdmin = isLoggedIn && user.user.username === "admin"; // Assuming you store user details in Redux
  ...
  const msg = async (e) => {
    e.preventDefault();
    if (!mobilenumber) {
      // Mobile number is not filled, show an error
    ...
    try {
      ...
      dispatch(setCart(updatedCart)); // Assuming you have a setCart action in your Redux store
      
      // Update local storage with the updated cart
      localStorage.setItem("cart", JSON.stringify(updatedCart));
    
      await axios.post(
        "http://localhost:8080/api/v1/auth/postbill",
        paymentData
        );
        dispatch(clearCart());
          ...
          <div id="bill12">
            <div id="bill-container12">
              <div id="billform12">
                ...
                <br />
                <label>
                  MOBILE NUMBER {mobilenumber.length === 0 && !mobilenumberError && (
